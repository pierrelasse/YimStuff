...
local function temp_text(infotext, duration)
    yu.rif(function(runscript)
        ...
        if objSpawner.infotextid == id then
            objSpawner.infotext = nil
        end
    end)
end
...
function renderObjectSpawner()
    ...
    local model_text, _ = ImGui.InputTextWithHint("Model",
                                                  "ex. stt_prop_stunt_bowling_pin",
                                                  objSpawner.model, 32)
    ...
    if objSpawner.infotext ~= nil then
        yu.rendering.coloredtext(objSpawner.infotext[1],
    ...
    if not objSpawner.blocked and ImGui.Button("Spawn") then
        yu.rif(function(runscript)
            ...
            local hash = yu.rendering.isCheckboxChecked(
                             "world_objspawner_hashmodel") and
                             joaat(objSpawner.model) or
                             tonumber(objSpawner.model)

            if hash == nil or not STREAMING.IS_MODEL_VALID(hash) or
                not STREAMING.IS_MODEL_A_VEHICLE(hash) then
                objSpawner.invalidmodel = true
            else
                STREAMING.REQUEST_MODEL(hash)
                repeat runscript:yield() until STREAMING.HAS_MODEL_LOADED(hash)

                if yu.rendering.isCheckboxChecked("world_objspawner_deleteprev") and
                    yu.does_entity_exist(objSpawner.entity) then
                    ENTITY.DELETE_ENTITY(objSpawner.entity)
                end

                local c = yu.coords(yu.ppid())
                objSpawner.entity = OBJECT.CREATE_OBJECT_NO_OFFSET(hash, c.x,
                                                                      c.y, c.z,
                                                                      true,
                                                                      yu.rendering
                                                                          .isCheckboxChecked(
                                                                          "world_objspawner_missionent") ~=
                                                                          false,
                                                                      true)
                ...
                if objSpawner.entity then
                    ...
                    ENTITY.FREEZE_ENTITY_POSITION(objSpawner.entity,
                                                  yu.rendering
                                                      .isCheckboxChecked(
                                                      "world_objspawner_freeze"))
                    if yu.rendering.isCheckboxChecked(
                        "world_objspawner_groundplace") then
    ...
    if objSpawner.entity ~= nil then
        ...
        if ImGui.Button("Delete##last_spawned") then
            yu.rif(function(runscript)
                if yu.does_entity_exist(objSpawner.entity) then
                    ENTITY.DELETE_ENTITY(objSpawner.entity)
                end
            end)
        end
    ...
    if ImGui.TreeNodeEx("Spawn options") then
        yu.rendering.renderCheckbox("Frozen", "world_objspawner_freeze",
                                    function(state)
            tasks.addTask(function()
                if objSpawner.entity ~= nil and
                    yu.does_entity_exist(objSpawner.entity) then
        ...
        yu.rendering.renderCheckbox("Delete previous",
                                    "world_objspawner_deleteprev")
        yu.rendering.renderCheckbox("Place on ground correctly",
                                    "world_objspawner_groundplace")
        yu.rendering.renderCheckbox("Mission entity",
                                    "world_objspawner_missionent")
...
function renderParticleSpawner()
    ImGui.PushItemWidth(parSpawner.awidth)

    local dict, _ = ImGui.InputTextWithHint("Dict", "ex. core",
                                            parSpawner.dict, 32)
    SussySpt.pushDisableControls(ImGui.IsItemActive())
    if parSpawner.dict ~= dict then parSpawner.dict = dict end

    local effect, _ = ImGui.InputTextWithHint("Effect",
                                              "ex. ent_sht_petrol_fire",
                                              parSpawner.effect, 32)
    SussySpt.pushDisableControls(ImGui.IsItemActive())
    ...
    if parSpawner.blocked ~= true and ImGui.Button("Spawn") then
        yu.rif(function(rs)
            ...
            repeat rs:yield() until STREAMING.HAS_NAMED_PTFX_ASSET_LOADED(
                parSpawner.dict)
            ...
            GRAPHICS.START_NETWORKED_PARTICLE_FX_NON_LOOPED_AT_COORD(
                parSpawner.effect, x, y, z, 90, -100, 90, 1, true, true,
                true, false)
